...
jslHelp={
	'Introduction':function(){
		...
		*	Why Choose JSL3 ?
		<br>
		*	<i>- Intuitive and Flexible Syntax</i> JSL is crafted to be easy for JavaScript developers to adopt. You can use familiar JavaScript expressions and builder patterns, reducing the need to learn new syntax and making templating straightforward.
		<br>
		*	<i>- Performance Focused</i> With asynchronous content generation, server-side serial output for chunked transfers, and optimized resource usage, JSL is engineered to handle high demands with minimal resource usage and remarkable speed, both in the browser and on the server.
		<br>
		*	<i>- Robust Features for Modern Development</i> JSL includes essential features for contemporary web applications, such as model-view notifications, multiple insert modes, and complete compatibility with in-browser debugging tools. JSL’s syntax flexibility allows you to mix JavaScript expressions with a builder pattern, adapting to your preferred style.
		<br>
		*	<i>- Seamless Debugging and Development</i> JSL eliminates the need for separate production builds and supports full compatibility with in-browser debuggers, helping developers save time and avoid potential errors.
		<br>
		*	What You Can Achieve with JSL3 ?
		<br>
		*	<i>With JSL</i> you can build fast, interactive, and maintainable web applications that leverage the full power of JavaScript. Whether you’re working on a single-page app or a complex, multi-page site, JSL provides the tools to create a responsive user experience with ease.
		<br>
		*	Key JSL Features at a Glance ?
		<br>
		*	- Native JavaScript-based templating
		*	- Model-View notifications for dynamic data updates
		*	- Asynchronous content generation for seamless UX
		*	- Multiple insert modes for custom content control
		*	- Server-side optimization, including serial output for chunked data
		*	- Fully compatible with browser debugging tools
		<br>
		*	Getting Started with JSL
		<br>
		*	Dive into JSL’s documentation to see how you can start building dynamic and efficient web applications. Whether you're new to templating or experienced with JavaScript frameworks, JSL offers the performance and flexibility you need to take your web development to the next level.
		*/
	},

	'Quick Start':function(){
		/**
		*	Welcome to JSL! This guide will get you set up and running in just a few steps, so you can start building efficient web applications.
		*	
		*	<b>Step 1: Installation</b>
		<br>
		*	Add JSL to your project by linking the library through a CDN:
		<br>
		* 	<pre style="background-color: white; color: #ed0c7d; font-size:20px; border-radius: 20px; padding: 20px">&Tab;&lt;script src="https://jsl3.codemax.net/jsl3.1.js"&gt;&lt;/script&gt;</pre> 
		<br>
		*	&Tab;<b>Step 2: Set Up Your First Template</b>
		<br>	
		<pre style="background-color: white; color: #ed0c7d; font-size:20px; border-radius: 20px">
		*	&Tab;&lt;!DOCTYPE html&gt
		*	&Tab;&lt;html lang="en"&gt
		*	&Tab;&lt;head&gt
		*	&Tab;&Tab;&lt;meta charset="UTF-8"&gt
		*	&Tab;&Tab;&lt;title&gtJSL Example&lt;/title&gt
		*	&Tab;&Tab;&lt;script src="http://jsl3.codemax.net/jsl3.1.js"&gt&lt;/script&gt
		*	&Tab;&lt;/head&gt
		*	&Tab;&lt;body&gt
		*	&Tab;&Tab;&lt;h1&gtWelcome to JSL&lt;/h1&gt
		*	&Tab;&lt;/body&gt
		*	&Tab;&lt;/html&gt
		</pre>
		*/
		...
		'A simple ToDo List':function(){
			/**
			*	A trivial todo list with JSL
			*/
	}[jsd].examples(function todoListExample(){//[title="ToDo list" example]
		/**	ToDo list example with Model-View separation*/			
		/*Model*/
		const todoList={//[collapsed][title=Todo list data model]	
			dochange:jsl.eventSource(/*@affected : array of items modified*/),
			doinsert:jsl.eventSource(/*@newItems : array of items inserted*/),
			tasks:[],tuid:0,
			add(task,silent){/** Add a new task
				*	@param 	task	string or HTMLInputElement
				*	@silent	when true do not trigger doinsert (batch operation) */
				let item={id:this.tuid++,task,done:false,doremove:jsl.eventSource()};
				this.tasks.push(item), silent || this.doinsert([item]);
				task.value='';
			},
			done(item,value){/** Mark the item as done/not done, and trigger dochange if there is a change */
				(item.done==value) || ((item.done=value) && this.dochange([item]));
			},
			archive(){/** Deletes the items marked as done */
				let deleted=this.tasks.filter(	(item,index) => 
					(item && item.done && (item.doremove(), delete this.tasks[index])) 
				);
				this.dochange(deleted);
			},
			count(){/**	Returns the number of non-deleted items*/
				return this.tasks.reduce( (r,item)=> (r+(!!item|0)), 0);
			},
			remaining(){/** Returns the items not marked as done*/
				return this.tasks.reduce( (r,item)=> (r+((item && !item.done)|0)), 0);
			}
		};	
		for(let i=0;i<10;i++){
			todoList.add(lorem(-8),'silent');
		};
		/*View*/
		jsl=
		MAIN({class:'todoList'},
			FORM(function onsubmit(e){//[title=add the new item in the todoList]
					e.preventDefault();
					todoList.add(this.elements.text.value);
					this.elements.text.value='';
					this.elements.text.focus();					
				},
				INPUT({name:'text',autocomplete:'off',type:'text',size:30,placeholder:'add new todo here',required:true}),
				INPUT({type:'submit',value:'add'})
			).$FORM,					
			SPAN({id:'todoListRemaining'},
				jsl.sinkFor(todoList.dochange,todoList.doinsert,null, c=>//c will be the eventSource, but we use it as a local var in the arrow-function;-)
					[STRONG(todoList.remaining()),' of ',c=todoList.count(),' ',c>1?'items':'item',' remaining']
				)
			),
			' [ ',A({onclick(e){todoList.archive();}},'Clear completed'),' ] ',						
			UL(//[title=populated UL with todoList items]
				jsl.sinkFor(todoList.doinsert,null,function(src,newItems=todoList.tasks){
					this.jsl.bof= newItems.map(item=>//map each todo list item to a LI element
						LI({id:'item'+item.id,class:'todoList-item',hidden:item.deleted},
							LABEL(
								INPUT({class:'done-checkbox',type:'checkbox',autocomplete:'off',checked:!!item.done},
									function onclick(event){
										todoList.done(item,this.checked);
									}
								),
								item.task
							).$LABEL,
							jsl.sinkFor(item.doremove,function(){//remove the list item when the todo item is removed from the todo list
								this.jsl.outer='';
							}),									
						).$LI
					)
